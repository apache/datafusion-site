...
these blog posts more regularly -- if you are interested in helping write them,
please reach out!
...
5. DataFusion meetups in multiple cities around the world: [Austin], [San Francisco], 
...
for many workloads. This feature should be available in future versions of DataFusion.
...
suite thanks to [@2010YOUY01] that has already found several bugs (kudos to [@jonahgao],
...
* Improved support for structured types such a  `STRUCT`, `LIST`/`ARRAY` and `MAP` 
...
You can also do even more complex things like parsing SQL, 
modifying the plan, and then converting it back to SQL:
...
With the rising prevalence of Parquet files stored on object stores, it is
important for query engines to support efficient access to Parquet files. Part
...
on row group level statistics as well as data page level statisics. DataFusion
now also supports users supplying their own low level pruning information via
the [`ParquetAccessPlan`] API.

This API can be used along with index informatio to selectively skip decoding
parts of the file. This feature has been used to add [efficient support] for
reading from DeltaLake tables and handling [deletion vectors].
...
Thanks to [@alamb] and XXX for this feature.  
...
As of DataFusion 40.0.0, the [`ExprPlanner`] API lets you provide a custom
implementation for planning expressions in SQL. This allows you to more
easily extend DataFusion's SQL planner to support custom operators or syntax.

For example [datafusion-functions-json] uses this API add support for JSON
operator support in SQL queries. It provides a custom implementation for planning
JSON operators such as `->` and `->>` with the following code:
...
Thanks to samuel colvin, jayzhan and others (TOD LINKS)
...
Huge thanks to [@milenkovicm] for this feature
...
The community has been [discussing what to work on in the next six months]. Some
major potential initiatives from contributors we know of this year are:
...
2. *Modularity*: Make DataFusion even more modular, by completing unifying
   built in and user [aggregate functions] and [window functions].

3. *LogicalTypes*: [Introducing Logical Types] would make it easier to use
...
[discussing what to work on in the next six months]: https://github.com/apache/datafusion/issues/11442
...
DataFusion is not a project that can be built by any one person or company. It
isn't supported by a single company or foundation, but rather by its community
of users and contributors.

If you are interested in joining the DataFusion community we would love to have
you. You can try out DataFusion on some of your own data and projects and let us
know how it goes, contribute suggestions, documentation, bug reports, or a PR
with documentation, tests or code. A list of open issues suitable for beginners
is [here] and you can find how to reach us on the [communication doc].
