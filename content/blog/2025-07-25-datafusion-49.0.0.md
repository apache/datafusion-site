...
### `WITHIN GROUP` support for Aggregate Functions
...
DataFusion now supports compressing spill files written to disk when spilling
intermediate results during query execution. This can significantly reduce the
size of spill files and improve performance when reading them back into memory.
...
DataFusion now supports the `REGEX_INSTR` function, which returns the position of a
regular expression match within a string. This function is useful for extracting
the position of a match in a string, similar to the [`REGEXP_INSTR` function].
