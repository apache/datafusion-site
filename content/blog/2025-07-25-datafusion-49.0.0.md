...
**Figure 2**: Planning benchmark performance improved XXX between DataFusion 48.0.0 and DataFusion 49.0.0. Chart source: TODO
...
(UDFs) in DataFusion. This allows users to write UDFs that can perform asynchronous
operations, such as network requests or database queries, without blocking the
execution of the query. We think this will allow new use cases for DataFusion, such as
integrating with large language models (LLMs) or other external services.
...
impl AsyncScalarUDFImpl for AskLLM {
    ...
    async fn invoke_async_with_args(
        ...
        headers.insert(AUTHORIZATION, get_llm_token(options));
...
(Issue [#6518](https://github.com/apache/datafusion/issues/6518), [PR #14837](https://github.com/apache/datafusion/pull/14837) from 
...
Sometomes it was not possible to cancel a long-running query in DataFusion, which could lead to resource exhaustion or unresponsiveness.

While this might sound like a small bug, the solution  requires a deep understanding of the DataFusion execution engine and the tokio execution model. 
The PR that implements it is a mdoel of careful community engineering as well as a great example of how to use Rust's async features to implement complex functionality.

See the blog [Using Rust async for Query Execution and Cancelling Long-Running Queries]: https://datafusion.apache.org/blog/2025/06/30/cancellation
https://github.com/apache/datafusion/pull/16398

It even results in a request upstream to tokio (TODO find).
