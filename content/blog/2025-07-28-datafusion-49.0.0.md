...
**Equivalence system upgrade:** The lower levels of the equivalence system, which is used to implement the 
  optimizations described in the [Using Ordering for Better Plans], was rewritten, leading to
...
DataFusion now supports for dynamic filters which improve during query execution 
...
find the top 10 values. With dynamic filters system, DataFusion applies an
...
technique drastically improves query performance (We've seen over 1.5x
improvement for some tpch style queries), especially in combination late
...
The last few months, between `46.0.0` and `49.0.0`, has seen our community grow:
...
leading to unresponsiveness. Other projects likely would have fixed this issue
by treating the symptom, but [pepijnve] and the DataFusion community worked together to
treat the root cause. The general solution required a deep understanding of
DataFusion execution engine, Rust `Streams`, and the tokio cooperative
scheduling model, and the [resulting PR] is a model of careful
...
documenting and testing, and would
...
Recently some users have reported success automaticlly upgrading DataFusion by
pairing AI tools and the upgrade guide. For a comprehensive list of all changes,
