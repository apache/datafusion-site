...
additions include significant performance improvements, 
...
([#20002], [#21668]), making planning 1.3x faster for filter-heavy queries
such as TPC-H Q19.
...
values). The inputs to queries are often range partitioned in real-world scenarios such as time-series where the data
is written one file per day or hour. DataFusion uses range partitioning information to
...
empty input as `NULL`s rather than dropping them.
