...
filtering window functions' output (similarly to how `HAVING` filters
aggregations' output).

For example, to filter the output of the `rank()` function we previously
...
behavior that varies based on runtime state. For example, time UDFs can now use
session specified time zones instead of just UTC.
...
users desire Spark compatible functionality in their workloads. Therefore, the
new release of DataFusion adds many new such functions, namely:
